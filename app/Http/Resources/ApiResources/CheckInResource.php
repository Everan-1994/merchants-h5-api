...
class CheckInResource extends Resource
{
    ...
    private function checkStatus($status, $check_in_time, $check_in_times)
    {
        switch ($status) {
            case 0:
                if ($check_in_times == 7 && Carbon::today()->gt(Carbon::parse(Carbon::parse($check_in_time)->toDateString()))) {
                    return -1; // 已过期
                }

                if ($check_in_times == 7 && Carbon::today()->eq(Carbon::parse(Carbon::parse($check_in_time)->toDateString()))) {
                    return 1; // 待抽奖
                }
